...
// Epic 3.2: Export Throttle (Anti-Scraping)
// Limits bulk data extraction to 5 requests per hour per IP/User
const exportLimiter = rateLimit({
  ...
  legacyHeaders: false,
  handler: (req, res) => {
    logger.warn(`Export rate limit exceeded by ${req.ip}`);
    res.status(429).json({ error: 'Çox sayda toplu məlumat yükləmə cəhdi. 1 saat sonra yenidən cəhd edin.' });
  }
});
  ...
  router.get('/', authenticate, authorize(`${permissionPrefix}:read`),
    async (req: AuthRequest, res: Response, next: NextFunction) => {
      try {
        ...
        // Epic 3.2: Apply export throttle if requesting large amount of data
        if (requestedLimit > 50) {
          exportLimiter(req, res, proceedWithQuery);
        } else {
          await proceedWithQuery();
        }

      } catch (error) {
        res.status(500).json({ error: 'Siyahı xətası' });
      }
    }
  );
